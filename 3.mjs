...
const sum = file.split('\n').reduce((memo, line, index, lines) => {
	...
	const lineTotal = line.split('').reduce((memo, c, idx, chars) => {
		if(c.match(/\d/) && memo.startIndex === undefined) memo.startIndex = idx
		else if(!c.match(/\d/) && memo.startIndex !== undefined) {
			memo.endIndex = idx
			if([].concat(
				index > 0 ? lines[index-1].slice(memo.startIndex - 1, memo.endIndex + 1) : [],
				memo.startIndex > 0 ? chars[memo.startIndex-1] : [],
				memo.endIndex < chars.length ? chars[memo.endIndex] : [],
				index < lines.length ? lines[index+1].slice(Math.max(0, memo.startIndex - 1), memo.endIndex + 1) : []
			)
			.some((c, _, arr) => c.match(/[^0-9.]/))) {
